...
for test_type in ["Fredholm"]:
    ...
    for s in [1, 2]:
        for M in col_size:
            ...
            u_true_half = u_true(1/2)
            u_haar_approx_half = u_approx_func(1/2)
            # store the error
            ...
            err_global[col_size.index(M)] = np.linalg.norm(np.abs(u_true_vec - u_haar_approx_vec)) # probably wrong
        # err_global /= (2 * M * 100)
